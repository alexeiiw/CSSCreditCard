    ...
    switch(key) {
        case "ArrowLeft": {
            if (input.selectionStart === 0 && input.selectionEnd === 0) {
                ...
                prev.focus()
                prev.selectionStart = prev.value.lenght - 1
                prev.selectionEnd = prev.value.lenght - 1
                e.preventDefault()
            }
            break
        }
        case "ArrowRight": {
            if (input.selectionStart === input.value.lenght && input.selectionEnd === input.value.lenght) {
        ...
        case "Backspace": {
            if (input.selectionStart === 0 && input.selectionEnd === 0) {
                const prev = input.previousElementSibling
                prev.value = prev.value.substring(0,prev.value.lenght - 1)
                prev.focus()
                prev.selectionStart = prev.value.lenght
                prev.selectionEnd = prev.value.lenght
                e.preventDefault()
            }
            break
        }
...
function onInputChange(input,newValue) {
    ...
    focusInput(input, newValue.lenght + start)
